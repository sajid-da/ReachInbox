...
export default function App() {
  ...
  const load = async () => {
    ...
    try {
      const [scheduledResponse, sentResponse] = await Promise.all([fetch(api('/api/emails/scheduled')), fetch(api('/api/emails/sent'))]);
  ...
  const activeRows = view === 'sent' ? sent : scheduled;

  return <div className="app-shell">
    <aside className="sidebar">
      <div className="brand">onB</div>
      <div className="profile-card"><div className="avatar">OB</div><div className="profile-copy"><strong>Oliver Brown</strong><span>oliver.brown@domain.io</span></div><Icon name="chevron" size={14}/></div>
    ...
    <main className="main-panel">
      {view === 'compose' ? <Compose onBack={() => setView('scheduled')} onScheduled={afterSchedule} /> : view === 'detail' && selected ? <Detail email={selected} onBack={() => setView(selected.status === 'sent' ? 'sent' : 'scheduled')} /> : <>
        <header className="topbar"><div className="search-box"><Icon name="search"/><input aria-label="Search" placeholder="Search" /></div><div className="top-actions"><button aria-label="Filter"><Icon name="filter"/></button><button aria-label="Refresh" onClick={() => void load()}><Icon name="refresh"/></button><div className="mini-avatar">OB</div></div></header>
...
function Compose({ onBack, onScheduled }: { onBack: () => void; onScheduled: (message: string) => void }) {
  const [recipients, setRecipients] = useState<string[]>([]); const [recipientInput, setRecipientInput] = useState(''); const [subject, setSubject] = useState(''); const [body, setBody] = useState(''); const [delay, setDelay] = useState('2'); const [hourlyLimit, setHourlyLimit] = useState('200'); const [startTime, setStartTime] = useState(''); const [sendLaterOpen, setSendLaterOpen] = useState(false); const [sending, setSending] = useState(false); const [error, setError] = useState('');
  ...
  const parseFile = (event: ChangeEvent<HTMLInputElement>) => { const file = event.target.files?.[0]; if (!file) return; const reader = new FileReader(); reader.onload = () => { const found = String(reader.result).match(/[\w.+-]+@[\w.-]+\.[A-Za-z]{2,}/g) ?? []; found.forEach(addRecipient); }; reader.readAsText(file); };
  const schedule = async (event?: FormEvent) => { event?.preventDefault(); if (!recipients.length || !subject.trim() || !body.trim()) { setError('Add at least one recipient, a subject, and a message.'); return; } setSending(true); setError(''); try { const response = await fetch(api('/api/emails/schedule'), { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ recipients, subject, body, startTime: startTime || new Date().toISOString(), delayMs: Number(delay) * 1000, hourlyLimit: Number(hourlyLimit), idempotencyKey: `campaign-${Date.now()}` }) }); if (!response.ok) { const data = await response.json().catch(() => null); throw new Error(data?.error ?? 'Unable to schedule campaign'); } onScheduled(`${recipients.length} email${recipients.length === 1 ? '' : 's'} scheduled`); } catch (e) { setError(e instanceof Error ? e.message : 'Unable to schedule campaign'); } finally { setSending(false); } };
  return <section className="compose-page"><header className="compose-header"><button className="back-button" onClick={onBack}><Icon name="back"/> <span>Compose New Email</span></button><div className="compose-actions"><label className="icon-action" title="Attach list"><Icon name="paperclip"/><input type="file" accept=".csv,.txt,text/csv,text/plain" onChange={parseFile}/></label><button className="icon-action" title="Send later" onClick={() => setSendLaterOpen((value) => !value)}><Icon name="clock"/></button><button className="send-button" onClick={() => void schedule()} disabled={sending}>{sending ? 'Sending…' : 'Send'}</button>{sendLaterOpen && <SendLater startTime={startTime} setStartTime={setStartTime} onDone={() => setSendLaterOpen(false)} onCancel={() => setSendLaterOpen(false)}/>}</div></header>
    <form className="compose-form" onSubmit={schedule}><div className="form-line"><label>From</label><button type="button" className="sender-select">oliver.brown@domain.io <Icon name="chevron" size={13}/></button></div><div className="form-line recipient-line"><label>To</label><div className="recipient-box">{recipients.map((recipient) => <span className="recipient-chip" key={recipient}>{recipient}<button type="button" onClick={() => setRecipients((current) => current.filter((item) => item !== recipient))}><Icon name="close" size={11}/></button></span>)}<input value={recipientInput} onChange={(event) => setRecipientInput(event.target.value)} onKeyDown={onRecipientKey} onBlur={() => recipientInput && addRecipient(recipientInput)} placeholder={recipients.length ? 'Add recipient' : 'recipient@example.com'}/><label className="upload-link"><Icon name="upload" size={13}/> Upload List<input type="file" accept=".csv,.txt,text/csv,text/plain" onChange={parseFile}/></label></div></div><div className="form-line"><label htmlFor="subject">Subject</label><input id="subject" value={subject} onChange={(event) => setSubject(event.target.value)} placeholder="Subject"/></div><div className="settings-line"><label>Delay between 2 emails <input type="number" min="0" value={delay} onChange={(event) => setDelay(event.target.value)}/></label><label>Hourly Limit <input type="number" min="1" value={hourlyLimit} onChange={(event) => setHourlyLimit(event.target.value)}/></label><label className="start-time">Start time <input type="datetime-local" value={startTime} onChange={(event) => setStartTime(event.target.value)}/></label></div><div className="editor"><input className="editor-placeholder" value={body} onChange={(event) => setBody(event.target.value)} placeholder="Type Your Reply..."/><div className="editor-toolbar"><button type="button"><Icon name="back"/></button><button type="button"><Icon name="back"/></button><span/><button type="button"><Icon name="bold"/></button><button type="button"><Icon name="italic"/></button><button type="button"><Icon name="underline"/></button><span/><button type="button"><Icon name="align"/></button><button type="button"><Icon name="list"/></button><button type="button"><Icon name="quote"/></button></div><textarea aria-label="Email body" value={body} onChange={(event) => setBody(event.target.value)} /></div>{error && <div className="form-error">{error}</div>}<button className="mobile-send primary-button" disabled={sending}>{sending ? 'Scheduling…' : 'Schedule emails'}</button></form>
  </section>;
}
...
function Detail({ email, onBack }: { email: Email; onBack: () => void }) { return <section className="detail-page"><header className="detail-header"><button className="back-button" onClick={onBack}><Icon name="back"/> <span>{email.subject || 'Email details'}</span></button><div className="detail-actions"><button><Icon name="star"/></button><button><Icon name="archive"/></button><button><Icon name="trash"/></button><div className="mini-avatar">OB</div></div></header><article className="message-card"><div className="message-meta"><div className="sender-avatar">{initials(email.email)}</div><div><strong>{email.email}</strong><span>to me · ReachInbox</span></div><time>{formatTime(email.sentTime ?? email.scheduledTime)}</time></div><div className="message-body"><p className="message-kicker">{email.status === 'sent' ? 'Delivered message' : 'Queued message'}</p><h2>{email.subject || '(no subject)'}</h2><p>This message is managed by your ReachInbox scheduler.</p><div className="highlight-box">{email.status === 'sent' ? 'Your email was sent successfully.' : 'This email is waiting in the delivery queue.'}</div><p className="message-muted">Recipient: {email.email}</p></div></article></section>; }
